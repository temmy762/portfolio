// scripts/generate-netlify-sitemap.mjs
import fs from 'fs';
...
// Configuration
const domain = process.env.SITE_URL || 'https://your-netlify-site.netlify.app'; // Update with your Netlify URL
...
async function generateSitemap() {
  console.log('Generating sitemap for Netlify deployment...');
  ...
  function scanDirectory(directory, basePath = '') {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        // Ensure trailing slash for Netlify format
  ...
  if (!fs.existsSync(robotsPath)) {
    const robotsTxt = `# robots.txt for Netlify deployment
User-agent: *
Allow: /
...
`;
    ...
    console.log('robots.txt created for Netlify');
  }
}
